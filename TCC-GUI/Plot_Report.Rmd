...
if(mdsMain == "Nonmetric MDS"){
  cat("library(MASS)\n")
  cat("mds <- data.frame(isoMDS(dist(1 - cor(data, method = '", mdsMethod, "'), method = '", mdsDistMethod, "')))\n")
} else {
  cat("mds <- data.frame(cmdscale(dist(1 - cor(data, method = '", mdsMethod, "'), method = '", mdsDistMethod, "')))\n")
}
cat("
...
plot_ly(data = mdsJ,
        ...
        mode = 'text',
        text =~ name,
       color =~ group) %>% layout(title = '", mdsMain, "Plot')
```\n")
params$mdsPlotplot
} else {
  cat("Not available.")
